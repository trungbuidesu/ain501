...
def benchmark_isolated_movinet():
    ...
    dummy_input = np.random.rand(1, 3, n_frames, resolution, resolution).astype(np.float32)
    ...
    print(f"Model Size on Disk:    {Path(model_path).stat().st_size / 1024 / 1024:.2f} MB")
